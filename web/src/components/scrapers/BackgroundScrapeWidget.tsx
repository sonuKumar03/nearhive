...
import { useScrapeJob, useCancelScraper } from '@/hooks/useScrapeJobs';
...
export default function BackgroundScrapeWidget({
  ...
  const completedTasks = tasks.filter((t) => t.status === 'done' || t.status === 'cancelled' || t.status === 'failed').length;
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-400">
              Hub: <span className="font-semibold text-slate-200">{job.region || 'Region'}</span>
            </p>
          </div>
        </div>
      ...
      <div className="mt-3 space-y-2">
        ...
        <div className="p-2.5 rounded-xl bg-slate-950/70 border border-slate-800/80 flex items-center justify-between text-xs">
          ...
          <span className="font-mono font-bold text-amber-400 text-sm">
            {job.sightings}{' '}
            <span className="text-[10px] font-normal text-slate-400">records</span>
          </span>
        </div>
      ...
      <div className="mt-3 pt-2.5 border-t border-slate-800/80 flex items-center justify-between gap-2">
        {isRunning ? (
          ...
          <span className="text-[10px] text-slate-500">
            {isDone ? 'Map markers refreshed' : 'Job finished'}
          </span>
        )}
